...
```php
'providers' => [
    Igorsgm\Likeable\Providers\LikeableServiceProvider::class,
],
```
...
```sh
$ php artisan vendor:publish --provider="Igorsgm\Likeable\Providers\LikeableServiceProvider" --tag=migrations
$ php artisan migrate
```
...
On each like added `\Igorsgm\Likeable\Events\ModelWasLiked` event is fired.

On each like removed `\Igorsgm\Likeable\Events\ModelWasUnliked` event is fired.

On each dislike added `\Igorsgm\Likeable\Events\ModelWasDisliked` event is fired.

On each dislike removed `\Igorsgm\Likeable\Events\ModelWasUndisliked` event is fired.
...
```php
$this->app->bind(
    \Igorsgm\Likeable\Contracts\Like::class,
...
```php
$this->app->singleton(
    \Igorsgm\Likeable\Contracts\LikeableService::class,
...
```php
$model = app(\Igorsgm\Likeable\Contracts\Like::class);
$service = app(\Igorsgm\Likeable\Contracts\LikeableService::class);
```
